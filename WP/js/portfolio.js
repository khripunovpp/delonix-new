...
var Portfolio = {
    ...
    _buildItem: function(item) {
        return '<a href="' + item.link + '" class="portfolio__item ' + Util.slugify(Util.translit(item.type)) + '"><img src="' + item.link + '" /></a>';
    },
    ...
    _renderFilter: function(uniqs) {
        ...
        function _createButton(type) {
            ...
            type = type ? '.' + type : '*';
            $(filterBut).addClass('filter__but').attr('data-filter', Util.slugify(Util.translit(type))).text(label)
            return filterBut
        }
